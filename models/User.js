...
const Profile = require("./Profile"); // Ensure this path is correct
...
const UserSchema = new User({
  ...
  birthday: { type: Date,Default:12  ,required: [false, "Birthday is required"], unique:false },
  password: {
    type: String,
    required: true,
  },
...
UserSchema.post("save", async function (doc, next) {
  try {
    ...
    if (!existingProfile) {
      const profile = new Profile({ userId: doc._id });
      await profile.save();
    }
    next();
  } catch (error) {
